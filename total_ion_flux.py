...
df = pd.read_csv('Output/sphere_flux_helio_temp.csv')
df = df.sort_values('Unnamed: 0')
df_helio = df.set_index('Unnamed: 0')
...
dfs = {'fluid':df_mfluid, 'species':df_mspecies, 'helio':df_helio, 'rhybrid':df_rhybrid}
colors = {'fluid':'DodgerBlue', 'species':'LightSkyBlue', 'helio':'MediumVioletRed', 'rhybrid':'Orchid'}
'$\mathrm{Ion\;Flux}$'

f, axes = plt.subplots(2,1)
for ax, ion in zip(axes, [ 'O2_p1', 'O_p1']):
    for dsk in ['fluid', 'species', 'helio', 'rhybrid']:
